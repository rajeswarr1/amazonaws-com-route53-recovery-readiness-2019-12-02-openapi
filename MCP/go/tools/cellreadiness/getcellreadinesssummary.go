...
func GetcellreadinesssummaryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/cellreadiness/%s%s", cfg.BaseURL, cellName, queryString)
		...
		var result models.GetCellReadinessSummaryResponse
...
func CreateGetcellreadinesssummaryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_cellreadiness_cellName",
		mcp.WithDescription("Gets readiness for a cell. Aggregates the readiness of all the resources that are associated with the cell into a single value."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetcellreadinesssummaryHandler(cfg),
	}
}
