...
func DeletecellHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletecellTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_cells_cellName",
		mcp.WithDescription("Delete a cell. When successful, the response code is 204, with no response body."),
		mcp.WithString("cellName", mcp.Required(), mcp.Description("The name of the cell.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletecellHandler(cfg),
	}
}
