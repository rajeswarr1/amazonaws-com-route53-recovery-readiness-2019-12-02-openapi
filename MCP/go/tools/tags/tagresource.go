...
func CreateTagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_tags_resource-arn",
		mcp.WithDescription("Adds a tag to a resource."),
		mcp.WithString("resource-arn", mcp.Required(), mcp.Description("The Amazon Resource Name (ARN) for a resource.")),
		mcp.WithObject("tags", mcp.Required(), mcp.Description("Input parameter: A collection of tags associated with a resource.")),
	)
