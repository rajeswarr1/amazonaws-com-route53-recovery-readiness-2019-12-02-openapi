...
func UntagresourceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/tags/%s#tagKeys%s", cfg.BaseURL, resource_arn, queryString)
...
func CreateUntagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_tags_resource-arn#tagKeys",
		mcp.WithDescription("Removes a tag from a resource."),
		mcp.WithString("resource-arn", mcp.Required(), mcp.Description("The Amazon Resource Name (ARN) for a resource.")),
		mcp.WithArray("tagKeys", mcp.Required(), mcp.Description("The keys for tags you add to resources.")),
	)
