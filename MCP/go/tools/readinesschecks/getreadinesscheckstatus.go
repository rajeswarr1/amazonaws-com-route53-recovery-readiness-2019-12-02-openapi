...
func GetreadinesscheckstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/readinesschecks/%s/status%s", cfg.BaseURL, readinessCheckName, queryString)
		...
		var result models.GetReadinessCheckStatusResponse
...
func CreateGetreadinesscheckstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_readinesschecks_readinessCheckName_status",
		mcp.WithDescription("Gets the readiness status for an individual readiness check. To see the overall readiness status for a recovery group, that considers the readiness status for all the readiness checks in a recovery group, use GetRecoveryGroupReadinessSummary."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetreadinesscheckstatusHandler(cfg),
	}
}
